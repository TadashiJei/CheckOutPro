...
async function updateProductImages() {
  try {
    ...
    for (const product of productImages) {
      // Download the image
      const localImageUrl = await downloadImage(product.imageUrl, product.localPath);
      
      if (localImageUrl) {
        // Update the product in the database
        await pool.execute(
          'UPDATE products SET image_url = ? WHERE name = ?',
          [localImageUrl, product.name]
        );
        
        console.log(`Updated product image for ${product.name}`);
      }
    }
